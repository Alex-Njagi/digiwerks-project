import { Modal, ModalOverlay, ModalContent, ModalBody, Flex, Box, Image, VStack, HStack, Text, Divider, Heading, Button } from "@chakra-ui/react";
        ...
        <ModalBody p={0}>
          <Flex h="90vh">            
            <Flex flex="1" bg="black" align="center" justify="center">
              <Image src= {version.file_url} alt="Version artwork" maxH="100%" maxW="100%" objectFit="contain"/>
            </Flex>            
            <Flex flex="1" direction="column" p={6} overflowY="auto">
              <VStack align="stretch" spacing={4}>                
                <Box>
                  <Heading size="md" color="brand.pink">{assetName}</Heading>
                  <Text fontSize="sm" color="gray.500">Version Number: {version.version_number}</Text>
                  <Text fontSize="sm" color="brand.blue">{artistName} — {projectTitle}</Text>
                </Box>
                <Divider borderColor="brand.pink"/>
                <Box>
                  <Text fontWeight="semibold" mb={1}>Description</Text>
                  <Text fontSize="sm" color="gray.700">{version.change_notes}</Text>
                </Box>
                <Button size="sm" bg="brand.pink" color="white" _hover={{ bg: "brand.blue" }} onClick={handleDelete} isLoading={deleteLoading} 
                loadingText="Deleting...">Delete Version</Button>
                {(deleteError) && (<Text color="red.500" fontSize="sm">{deleteError?.message}</Text>)} 
